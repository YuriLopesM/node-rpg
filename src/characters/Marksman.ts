...
export class Marksman extends Character {
  ...
  attack() {
    ...
    this.nextDmg = dmg;
    return `${this.name} fires an projectile! Dmg: ${dmg}.`;
  }
  ...
  defend() {
    ...
    return `${this.name} takes cover! Reduced damage: ${this.nextDmg}.`;
  }
  ...
  castSpell() {
    ...
    if (isCrit) {
      ...
      return `${this.name} lands a critical hit! Dmg: ${critDmg}.`;
    } else {
      return `${this.name} fires a precise shot! Dmg: ${this.nextDmg}.`;
    }
  }
  ...
  castUltimateSpell() {
    ...
    return `${this.name} fires a shot directly through the head! Dmg: ${totalDmg}.`;
  }
}
